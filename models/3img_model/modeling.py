...
attention_scores_dict = {'lang': [], 'vision': [], 'cross': []}
batch = 0
...
class BertAttention(nn.Module):
    ...
    def forward(self, hidden_states, context, attention_mask=None, task=None):
        ...
        attention_scores_dict[task].append(attention_probs.cpu().detach().numpy())
...
class BertCrossattLayer(nn.Module):
    ...
    def forward(self, input_tensor, ctx_tensor, task, ctx_att_mask=None):

        output = self.att(input_tensor, ctx_tensor, ctx_att_mask, task=task)
...
class BertSelfattLayer(nn.Module):
    ...
    def forward(self, input_tensor, attention_mask, task):

        # Self attention attends to itself, thus keys and querys are the same (input_tensor).
        self_output = self.self(input_tensor, input_tensor, attention_mask, task=task)
...
class LXRTXLayer(nn.Module):
    ...
    def cross_att(self, lang_input, lang_attention_mask, visn_input, visn_attention_mask):
        # Cross Attention
        lang_att_output = self.visual_attention(lang_input, visn_input, 'cross', ctx_att_mask=visn_attention_mask)
        visn_att_output = self.visual_attention(visn_input, lang_input, 'cross', ctx_att_mask=lang_attention_mask)
        return lang_att_output, visn_att_output

    def self_att(self, lang_input, lang_attention_mask, visn_input, visn_attention_mask):
        # Self Attention
        lang_att_output = self.lang_self_att(lang_input, lang_attention_mask, 'lang')
        visn_att_output = self.visn_self_att(visn_input, visn_attention_mask, 'vision')
        return lang_att_output, visn_att_output
...
class LXRTEncoder(nn.Module):
    ...
    def forward(self, lang_feats, lang_attention_mask,
        ...
        with open('/content/drive/My Drive/att_scores_out_3img/sent_feats.txt', 'a+') as f:
            for item in lang_feats:
                f.write("%s\n" % item)

        # Run language layers
        for layer_module in self.layer:
            lang_feats = layer_module(lang_feats, lang_attention_mask, 'lang')

        # Run relational layers
        for layer_module in self.r_layers:
            visn_feats = layer_module(visn_feats, visn_attention_mask, 'vision')
        ...
        np.save('/content/drive/My Drive/att_scores_out_3img/'+str(batch)+'.npy', attention_scores_dict)
        batch += 1
        attention_scores_dict = {'lang': [], 'vision': [], 'cross': []}
