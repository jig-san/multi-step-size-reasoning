...
class NLVR2:
    ...
    def predict(self, eval_tuple: DataTuple, dump=None):
        ...
        keys = ['self_lang', 'self_vis', 'cross_lang', 'cross_vis']
        ...
        for i, datum_tuple in enumerate(loader):
            ...
            print(ques_id)# avoid handling target
            print(sent)
            with torch.no_grad():
                feats, boxes = feats.cuda(), boxes.cuda()
                logit = self.model(feats, boxes, sent)

                with open('/content/drive/My Drive/out_attention.csv', "a+", newline='') as outfile:
                    writer = csv.writer(outfile, delimiter=';')
                    for d in lxrt.modeling.out_final:
                        writer.writerow(list(d.values()))

                lxrt.modeling.out_final = []
    ...
    def evaluate(self, eval_tuple: DataTuple, dump=None):
        ...
        csvfilename = '/content/drive/My Drive/out_attention.csv'

        # if not os.path.exists(csvfilename):
        #     os.mknod(csvfilename)
