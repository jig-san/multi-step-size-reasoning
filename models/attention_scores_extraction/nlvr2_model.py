...
class NLVR2Model(nn.Module):
    ...
    def forward(self, feat, pos, sent):
        ...
        sent = sum(zip(sent, sent, sent), ())
        batch_size, img_num, obj_num, feat_size = feat.size()
        assert img_num == 3 and obj_num == 36 and feat_size == 2048
        feat = feat.view(batch_size * 3, obj_num, feat_size)
        pos = pos.view(batch_size * 3, obj_num, 4)

        # Extract feature --> Concat
        x = self.lxrt_encoder(sent, (feat, pos))
        
        x = x.view(-1, self.hid_dim*3)
